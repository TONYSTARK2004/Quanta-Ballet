...
def run_second_script():
    ...
    subprocess.run(["python", second_script_path])
    subprocess.run(["python", "guic.py"])
...
name_label = tk.Label(root, text="Enter your uid:")
name_label.pack()
...
# Submit button
submit_button = tk.Button(root, text="Verify Fingerprint", command=on_submit)
submit_button.pack()

# Button to run the second script
run_script_button = tk.Button(root, text="Go For Vote.", command=run_second_script)
